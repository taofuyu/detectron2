...
sys.path.append("/data/taofuyu/repos/detectron2/projects/CrowdDet/lib")
sys.path.append("/data/taofuyu/repos/detectron2/projects/CrowdDet/model")
sys.path.append("/data/taofuyu/repos/detectron2/projects/CrowdDet/model/retina_emd_simple_H1M")
from data.CrowdHuman import CrowdHuman
from data.H1M_dataset import H1MDataset
...
def train_worker(rank, train_config, network, config):
    ...
    # build data provider
    h1mdataset = H1MDataset(config, if_train=True)
    data_iter = torch.utils.data.DataLoader(dataset=h1mdataset,
            batch_size=train_config.mini_batch_size,
            num_workers=2,
            collate_fn=h1mdataset.merge_batch,
            shuffle=True)
...
def multi_train(params, config, network):
    ...
    train_config.learning_rate = config.base_lr
    train_config.momentum = config.momentum
    train_config.weight_decay = config.weight_decay
    train_config.lr_decay = config.lr_decay
    train_config.model_dir = os.path.join('/data/taofuyu/snapshot/crowed/', params.model_dir, config.model_dir)
    line = 'network.lr.{}.train.{}'.format(
            train_config.learning_rate, train_config.total_epoch)
    train_config.log_path = os.path.join('/data/taofuyu/snapshot/crowed/', params.model_dir, config.output_dir, line+'.log')
...
def run_train():
    ...
    parser.add_argument('--model_dir', '-md', default="retina_fpn_baseline_H1M",required=False,type=str)
    ...
    # import libs
    model_root_dir = os.path.join('/data/taofuyu/repos/detectron2/projects/CrowdDet/model', args.model_dir)
