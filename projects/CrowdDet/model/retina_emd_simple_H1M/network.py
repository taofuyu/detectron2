...
from backbone.h1m import H1Mbackbone
...
class Network(nn.Module):
    def __init__(self):
        super().__init__()
        self.h1mbackbone = H1Mbackbone()
        self.FPN = FPN(self.h1mbackbone, 2, 6)
...
class RetinaNet_Criteria(nn.Module):
    ...
    def __call__(self, pred_cls_list, pred_reg_list, anchors_list, gt_boxes, im_info):
        ...
        all_pred_cls = torch.cat(pred_cls_list, axis=1).reshape(-1, (config.num_classes)*2)
        ...
        all_pred_cls = all_pred_cls.reshape(-1, 2, config.num_classes)
...
class RetinaNet_Head(nn.Module):
    def __init__(self):
        ...
        self.cls_score = nn.Conv2d(
            in_channels, config.num_cell_anchors * (config.num_classes) * 2,
    ...
    def forward(self, features):
        ...
        pred_cls_list = [
            _.permute(0, 2, 3, 1).reshape(pred_cls[0].shape[0], -1, (config.num_classes)*2)
            for _ in pred_cls]
        pred_reg_list = [
