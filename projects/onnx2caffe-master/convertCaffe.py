...
import sys
sys.path.append("/data/taofuyu/cafferaw_det/python/")
import caffe
...
def reload_model(old_weight, new_deploy, new_caffemodel):
    net = caffe.Net(new_deploy, old_weight, caffe.TEST)
    net.save(new_caffemodel)
    # a=0
    # layers = net.layer_dict
    # layer_in_name = net.bottom_names
    # layer_out_name = net.top_names
    
    # layer_idx = -1
    # for i,l in enumerate(layer_in_name):
    #     if "Conv" in l:
    #         layer_idx = i
    #model = caffe.Net(new_deploy, new_caffemodel, caffe.TEST)

    

if __name__ == "__main__":
    onnx_path = "/data/taofuyu/snapshot/FCOS/X5/fcos_X5.onnx"
    prototxt_path = "/data/taofuyu/snapshot/FCOS/X5/deploy_fcos_X5.prototxt"
    caffemodel_path = "/data/taofuyu/snapshot/FCOS/X5/weight_fcos_X5.caffemodel"

    #with post process
    new_deploy = "/data/taofuyu/snapshot/FCOS/X4/deploy_fcos_X4_post.prototxt"
    new_caffemodel = "/data/taofuyu/snapshot/FCOS/X4/weight_fcos_X4_post.caffemodel"

    graph = getGraph(onnx_path)
    convertToCaffe(graph, prototxt_path, caffemodel_path)
    
    #reload_model(caffemodel_path, new_deploy, new_caffemodel)
